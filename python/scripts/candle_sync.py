...
def _load_generate_candlestick_image():
    module_path = Path(__file__).resolve().parents[1] / "src" / "data" / "image_generator.py"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project_id", default=os.getenv("BIGQUERY_PROJECT"))
    parser.add_argument("--dataset_id", default=os.getenv("BIGQUERY_DATASET"))
    parser.add_argument("--bucket", default=os.getenv("CANDLE_BUCKET", "boostcamp-final-proj"))
    parser.add_argument("--symbols", nargs="*", default=None)
    parser.add_argument("--windows", nargs="*", type=int, default=None)
    parser.add_argument("--chart_type", default=os.getenv("CANDLE_CHART_TYPE", "candle"))
    parser.add_argument("--image_size", type=int, default=int(os.getenv("CANDLE_IMAGE_SIZE", "224")))
    parser.add_argument(
        "--dotenv_path",
        default=os.getenv("AIRFLOW_ENV", "/data/ephemeral/home/airflow/.env"),
    )
