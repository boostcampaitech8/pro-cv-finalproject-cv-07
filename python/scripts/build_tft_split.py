...
python scripts/build_tft_split.py \
  ...
  --val_months 2 \
  --output_dir src/datasets/bq_splits/deploy
...
python scripts/build_tft_split.py \
  ...
  --target_commodity corn \
  --val_months 3 \
  --output_dir src/datasets/local_splits/eval
"""
...
def _load_price_df(cfg: SplitConfig) -> pd.DataFrame:
    ...
    if not price_path.exists():
        # Fallback to global preprocessing dir: src/datasets/preprocessing/{commodity}_feature_engineering.csv
