...
from shared.data.bigquery_loader import COMMODITY_TO_SYMBOL, load_price_table
...
class CNNDataset(Dataset):
    ...
    def __init__(
        ...
        self.data_root = Path(data_dir) if data_dir else Path("shared/src/datasets")
    ...
    def _get_inference_dates(self, fold_data: Dict) -> List[str]:
        # Prefer inference_price dates (BigQuery/local) over split meta.
        source_dates = self._get_inference_dates_from_source()
        if source_dates:
            return source_dates
    ...
    def _get_inference_dates_from_source(self) -> List[str]:
        """
        Pull inference dates directly from inference_price table or CSV.
        This keeps inference aligned with the latest inference_price data,
        independent of train/val split windows.
        """
        try:
            if self.data_source == "bigquery":
                if not self.bq_project_id or not self.bq_dataset_id or not self.bq_inference_table:
                    return []
                df = load_price_table(
                    project_id=self.bq_project_id,
                    dataset_id=self.bq_dataset_id,
                    table=self.bq_inference_table,
                    commodity=self.commodity,
                )
            else:
                infer_path = self.data_root / "inference_price.csv"
