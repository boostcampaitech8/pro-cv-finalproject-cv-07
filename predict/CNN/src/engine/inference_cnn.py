...
from data.dataset_cnn import CNNDataset, cnn_collate_fn
from interpretation.cnn_visualizer import GradCAM
from models.CNN import CNN
from shared.utils.unified_output import build_anomaly_payload, map_severity_levels, write_json
...
def run_inference_cnn(
    ...
    Legacy outputs go to outputs/predictions/cnn, unified outputs to
    outputs/predictions/unified/cnn.
    """
    ...
    if prediction_root:
        ...
        data_path = data_root or Path(".").resolve()
    ...
    if write_csv:
        ...
        if csv_rows:
            ...
            df_new = df_new.dropna(subset=["predict_date"]).reset_index(drop=True)
            ...
            if csv_path.exists():
                ...
                # Deduplicate by key columns if present
                key_cols = [c for c in ["base_date", "window_size", "predict_date"] if c in df.columns]
                if key_cols:
