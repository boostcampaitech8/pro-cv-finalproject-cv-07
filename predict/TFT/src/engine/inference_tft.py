...
from shared.utils.unified_output import build_forecast_payload, map_horizon_values, write_json
...
def _resolve_output_root(
    ...
    return (
        Path("outputs/predictions/unified/tft")
        / commodity
        / exp_name
        / f"fold_{fold}"
        / split
...
def _window_dates_for_target(
    ...
    if start < 0:
        return None
    # Include base_date (idx) in the window.
    return all_dates[start : idx + 1]


def _prepare_horizon_groups(horizons: Sequence[int], groups: Sequence[int]) -> Dict[int, List[int]]:
    group_indices: Dict[int, List[int]] = {}
    if not groups:
        # Aggregate all horizons into a single group.
        if horizons:
            group_indices[int(max(horizons))] = list(range(len(horizons)))
...
def run_inference_tft(
    ...
    *,
    ...
    news_credentials_path: Optional[str] = None,
    split: str = "test",
    batch_size: int = 128,
    num_workers: int = 4,
    ...
    if news_source == "bigquery":
        ...
        news_data_source = load_news_features_bq(
            ...
            table=bq_news_table,
            commodity=commodity,
            credentials_path=news_credentials_path,
        )
    ...
    if output_root is None and split == "inference" and dates:
        data_path = Path(data_dir).resolve()
        predict_root = data_path
        if data_path.name == "datasets" and data_path.parent.name == "src" and data_path.parent.parent.name == "shared":
            predict_root = data_path.parent.parent.parent
        elif data_path.name == "datasets" and data_path.parent.name == "shared":
            predict_root = data_path.parent.parent
        elif data_path.name == "datasets":
            predict_root = data_path.parent
        output_root = (
            predict_root
            / "outputs"
            / "predictions"
            / f"{dates[-1]}_{commodity}_tft"
            / f"w{seq_length}"
            / "results"
        )
    ...
    # Align quantile settings with checkpoint to avoid shape mismatch.
    ckpt_quantile_loss = bool(model_cfg.get("quantile_loss", False))
    ckpt_quantiles = model_cfg.get("quantiles", None)
    effective_quantiles = list(quantiles) if quantiles is not None else None
    if not ckpt_quantile_loss:
        if effective_quantiles is not None:
            print("[WARN] Checkpoint trained without quantile_loss; ignoring quantiles in inference.")
        effective_quantiles = None
    else:
        if ckpt_quantiles is not None:
            ckpt_quantiles = [float(q) for q in ckpt_quantiles]
        if effective_quantiles is None and ckpt_quantiles:
            effective_quantiles = ckpt_quantiles
        elif effective_quantiles is not None and ckpt_quantiles and len(effective_quantiles) != len(ckpt_quantiles):
            print("[WARN] Quantiles mismatch with checkpoint; using checkpoint quantiles.")
            effective_quantiles = ckpt_quantiles
    ...
    for idx, date_str in enumerate(dates):
        ...
        if base_close is not None and np.isfinite(base_close):
            ...
            for h_idx, h in enumerate(horizons):
                predict_date = pred_date_map.get(int(h))
                if predict_date is None:
                    continue
                combined_rows.append(
                    {
                        "base_date": str(date_str)[:10],
                        "window_size": int(seq_length),
                        "predict_date": predict_date,
                        "predicted_close": close_pred[h_idx],
                    }
                )
    ...
    if return_rows:
        return output_root_path, combined_rows
    return output_root_path
