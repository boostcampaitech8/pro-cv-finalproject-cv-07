...
def align_news_to_trading_dates_runtime(
    ...
    raw_dates = news_df_copy['date']
    aligned_to_trading = raw_dates.notna().all() and set(raw_dates).issubset(trading_dates_set)

    if not aligned_to_trading:
...
def build_tft_dataset(
    ...
    if feature_columns is None:
        exclude_cols = ["time", "date", "trade_date", "symbol", "ingest_time"]
        exclude_cols += [c for c in time_series.columns if c.startswith("log_return_")]
        exclude_cols += [c for c in time_series.columns if c.startswith("vol_return_")]
        numeric_cols = [
    ...
    if require_targets:
        # Last usable base_date must still have max_horizon available.
        end_idx = len(time_series) - seq_length - max_h + 1
    else:
        # Inference: allow last window to end at final row.
        end_idx = len(time_series) - seq_length + 1

    for i in range(max(0, end_idx)):
        # 입력: t-(seq_length-1) ~ t (target_base = i+seq_length-1)
